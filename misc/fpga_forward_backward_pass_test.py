...
# conduct 10 gradient check tests
#for i in range(10):
#
...
#    print("Neuron " + str(i))
#    for j in range(98):
#        print(str(j) + ": " + str(fc1_w_grad[i][j]))
##
##
...
#    print("Neuron " + str(i))
#    for j in range(784):
#        print(str(j) + ": " + str(fc0_w_grad[i][j]))
...
for i in range(len(fc0_w_grad)):
    for grad in fc0_w_grad[i]:
        if grad != 0.0:
            no_zero_fc0.append(grad)

for i in range(len(fc1_w_grad)):
    for grad in fc1_w_grad[i]:
        if grad != 0.0:
            no_zero_fc1.append(grad)

for i in range(len(weight_grad)):
    for grad in weight_grad[i]:
        if grad != 0.0:
            no_zero_fc2.append(grad)

print('Fc0: ' + str(len(no_zero_fc0) / (fc0_n_neurons * fc0_fan_in)))
print('Fc1: ' + str(len(no_zero_fc1) / (fc1_n_neurons * fc1_fan_in)))
print('Fc2: ' + str(len(no_zero_fc2) / (fc2_n_neurons * fc2_fan_in)))

no_zero_fc0 = np.asarray(no_zero_fc0)
no_zero_fc1 = np.asarray(no_zero_fc1)
no_zero_fc2 = np.asarray(no_zero_fc2)

sns.distplot(no_zero_fc0, label='FC0', bins = 70, norm_hist=True)
sns.distplot(no_zero_fc1, label='FC1', bins = 70, norm_hist=True)
sns.distplot(no_zero_fc2, label='FC2', bins = 70, norm_hist=True)
plt.xlabel('Weight Value')
plt.ylabel('Number of weights in bin')
plt.legend()
plt.xlim(-0.06, 0.06)
plt.show()